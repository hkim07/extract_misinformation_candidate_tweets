...
NEXT collect their parents if their texts satisfy the query ((corona OR coronavirus OR covid-19 OR covid19 OR 2019-ncov OR wuhanvirus OR (wuhan AND virus)) AND (antibiotic OR antibiotics)). 
```python
import tweepy
api = tweepy.API(auth)
